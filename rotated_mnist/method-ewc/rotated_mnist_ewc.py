...
def train_ewc(model, train_loaders, test_loaders, device, args):
    ...
    for task_id, train_loader in enumerate(train_loaders):
        ...
        prev_ewcs.append(EWC(model, train_loader, device, args.lambda_ewc))
        # Each EWC object stores Fisher + reference params = 2 × params_bytes
...
if __name__ == '__main__':
    ...
    save_acc_matrix(acc_matrix, method_name="EWC", n_tasks=args.n_tasks, seed=args.seed)
    plot_acc_lines(acc_matrix, method_name="EWC", n_tasks=args.n_tasks)
    tracker.print_summary()
    tracker.save_report(f"reports/cost_EWC_{args.n_tasks}tasks_seed{args.seed}.csv")
