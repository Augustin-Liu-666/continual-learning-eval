...
class AGEM:
    ...
    def project_gradient(self, criterion):
        ...
        for p in self.model.parameters():
            current_grad.append(p.grad.data.view(-1).clone() if p.grad is not None
        ...
        ref_grad = []
        for p in self.model.parameters():
            ref_grad.append(p.grad.data.view(-1).clone() if p.grad is not None
                             else torch.zeros(p.numel(), device=self.device))
    ...
    def memory_bytes(self):
        if self.mem_x is None:
            return 0
        return (self.mem_x.element_size() * self.mem_x.numel() +
                self.mem_y.element_size() * self.mem_y.numel())
...
def train_agem(model, train_loaders, test_loaders, device, args):
    ...
    for task_id, train_loader in enumerate(train_loaders):
        ...
        tracker.end_task(task_id, extra_storage_bytes=agem.memory_bytes(),
                         extra_storage_label=f"episodic_memory ({len(agem.mem_x) if agem.mem_x is not None else 0} samples)")
...
def save_acc_matrix(acc_matrix, method_name, n_tasks, seed=0, memory_size=None):
    ...
    path = f"reports/acc_matrix_{method_name}_{n_tasks}tasks{mem_tag}_seed{seed}.csv"
    df.to_csv(path)
...
if __name__ == '__main__':
    ...
    save_acc_matrix(acc_matrix, method_name="AGEM", n_tasks=args.n_tasks, seed=args.seed, memory_size=args.memory_size)
    plot_acc_lines(acc_matrix, method_name="AGEM", n_tasks=args.n_tasks)
    tracker.print_summary()
    tracker.save_report(f"reports/cost_AGEM_{args.n_tasks}tasks_mem{args.memory_size}_seed{args.seed}.csv")
