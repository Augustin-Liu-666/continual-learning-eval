...
def create_split_mnist_tasks():
    ...
    for (class1, class2) in class_pairs:
        ...
        targets = (targets == class2).long()  # 将class2标记为1 / Label class2 as 1
...
class SimpleMLP(nn.Module):
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1)  # 扁平化输入 / Flatten input
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x

# 计算Fisher信息矩阵 / Compute Fisher Information Matrix
...
# EWC损失函数 / EWC loss function
def ewc_loss(model, criterion, output, target, ewc_lambda, old_params, fisher_matrix):
    loss = criterion(output, target)
    if old_params is not None:
        for n, p in model.named_parameters():
            loss += (ewc_lambda/2) * (fisher_matrix[n] * (p - old_params[n]).pow(2)).sum()
    return loss
...
ewc_lambda = 1000  # 正则化强度 / Regularization strength
...
for task_id, loader in enumerate(task_loaders):
    ...
    for epoch in range(10):
        for data, target in loader:
            ...
            optimizer.zero_grad()
            output = model(data)
            loss = ewc_loss(model, criterion, output, target, ewc_lambda, old_params, fisher_matrix)
            loss.backward()
            optimizer.step()
    
    # 更新Fisher矩阵 / Update Fisher
    fisher_matrix = compute_fisher(model, loader)
...
plt.ylabel('Accuracy (%)')
plt.title('Accuracy Evolution Across Tasks (EWC Training)')
plt.xticks([1,2,3,4,5])
plt.ylim(0,100)
plt.legend()
plt.grid(True)
plt.savefig('ewc_accuracy_curve.png')


print("\n✅ Accuracy curve saved as 'ewc_accuracy_curve.png'!")
...
# 打印指标 / Print metrics
print("\n📈 Evaluation Results (EWC Training):")
