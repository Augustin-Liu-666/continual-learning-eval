...
def train_ewc(model, train_loaders, test_loaders, device, args):
    ...
    for task_id, loader in enumerate(train_loaders):
        ...
        prev_ewcs.append(EWC(model, loader, device, args.lambda_ewc))
        tracker.end_task(task_id,
                         extra_storage_bytes=len(prev_ewcs) * 2 * params_bytes,
...
if __name__ == '__main__':
    ...
    acc_matrix, tracker = train_ewc(model, train_loaders, test_loaders, device, args)
    ...
    csv_path = f"cifar100/reports/acc_matrix_EWC_{args.n_tasks}tasks_seed{args.seed}.csv"
    df.to_csv(csv_path)
    tracker.print_summary()
    tracker.save_report(f"cifar100/reports/cost_EWC_{args.n_tasks}tasks_seed{args.seed}.csv")
    ...
    plt.title("Accuracy vs Task (CIFAR-100 – EWC)")
    ...
    plt.savefig(f"cifar100/reports/acc_plot_EWC_{args.n_tasks}tasks.png")
    plt.close()
